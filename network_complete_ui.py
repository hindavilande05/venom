...
class InternetConnectionsApp:
    ...
    def __init__(self, parent):  # Accept the parent widget (which will be a Frame)
        self.root = parent  # Use the passed parent as the root
        self.create_widgets()
    
    def create_widgets(self):
        self.processes_label = tk.Label(self.root, text="Processes with Active Internet Connections")
        self.processes_label.pack(pady=20)

        self.process_listbox = tk.Listbox(self.root, width=100)  
        self.process_listbox.pack()

        self.connections_label = tk.Label(self.root, text="Active Connections for Selected Process")
        self.connections_label.pack(pady=20)
